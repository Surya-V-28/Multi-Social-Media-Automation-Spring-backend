...
public class InstagramFeedPostValidator implements PostTargetValidator {
    @Override
    public List<String> validate(ValidatingPost validatingPost) {
        var validationErrors = new ArrayList<String>();

        if (validatingPost.caption().length() > 63206) {
            validationErrors.add("Max characters for caption = ");
        }
    ...
    private final Set<MimeType> imageFormats = Set.of(MimeType.valueOf("image/jpeg"), MimeType.valueOf("image/png"));
